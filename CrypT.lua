...
do
    local equiphigher_group = Misc2:AddLeftGroupbox('Item Level Req Bypass')
    equiphigher_group:AddLabel('Enabled by default. Equip items from inventory\nMade by Wally')
    ...
    if not CardinalClient then
        equiphigher_group:AddLabel('Executor incompatible ❌')
        return
    end
    ...
    local function FindSuitableInventoryItem(Target)
        for _, Item in next, PlrInventory:GetChildren() do
            ...
            if DatabaseEntry then
                if DatabaseEntry.Type.Value == Target.Type.Value then
                    return {
                        Name = Item.Name,
    ...
    equiphigher_group:AddLabel('✔ Item level bypass loaded\nEquip any item regardless of level.')
end
